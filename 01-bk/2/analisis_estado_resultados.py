...
def generar_estado_resultados_todos_los_meses(df_datos, archivo_parametros, centro_costos=None):
    resultados = []

    for año in sorted(df_datos["AÑO"].unique()):
        for mes in sorted(df_datos[df_datos["AÑO"] == año]["MES"].unique()):
            df_estado, _ = generar_estado_resultados_detallado(df_datos, archivo_parametros, año, mes, centro_costos)
            df_estado["AÑO"] = año
            df_estado["MES"] = mes
