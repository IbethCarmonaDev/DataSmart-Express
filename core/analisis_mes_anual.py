...
def OLDanalizar_mes_anual(df_kpis, df_mensual, df_anual, plan, config):
    frases = generar_conclusiones(df_kpis)
    ...
    if os.getenv("APP_ENV") == "DEV":
        print("⚠️ [MOCK] GPT desactivado. Modo desarrollo activo.")
        frases.insert(0, "[MOCK] Resumen simulado. GPT no se ejecutó.")
        return frases

    # Resumen narrativo con GPT
    if config.get("Resumen narrativo con GPT"):
        prompt = "Resume el siguiente análisis financiero de forma fluida:\n" + "\n".join(frases)
        try:
            client = _get_client()
            resp = client.chat.completions.create(
                model="gpt-3.5-turbo",
            ...
            # Mensaje amigable ante problemas de cuota o conexión
            frases.append(
                "⚠️ Resumen narrativo GPT no disponible por cuota o configuración. "
